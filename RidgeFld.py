...
class RectangleDetector:
    ...
    def extract_trapezoids(self, intersections, adjacency, image, debug_img=None, graph_black=None):
    ...
    def draw_quadrilaterals(self, img, quads, color):
        for quad in quads:
            for i in range(4):
                cv2.line(img, tuple(quad[i]), tuple(quad[(i + 1) % 4]), color, 1, cv2.LINE_AA)
    ...
    def process_image(self, image, output_folder, base_name):
            ...
            for i in range(len(intersections)):
                for j in range(i + 1, len(intersections)):
                    if adjacency[i][j]:
                        ...
                        cv2.line(graph_img, intersections[i], intersections[j], (255, 0, 255), 1, cv2.LINE_AA)
                        # Líneas magenta en graph_black, grosor 1 y antialiasing
                        cv2.line(graph_black, intersections[i], intersections[j], (255, 0, 255), 1, cv2.LINE_AA)
        ...
        debug_img = image.copy()
        # trapezoids = self.extract_trapezoids(intersections, adjacency, image, debug_img=debug_img, graph_black=graph_black)
        trapezoids, failed_angle_traps = self.extract_trapezoids(intersections, adjacency, image, debug_img=debug_img, graph_black=graph_black)
        ...
        output = image.copy()
        self.draw_quadrilaterals(output, rectangles, (255, 255, 255))  # Blanco para válidos
        if self.draw_trapezoids:
            self.draw_quadrilaterals(output, others, (0, 255, 255))  # Amarillo claro para otros válidos

        image_with_failed_angles = image.copy()
        self.draw_quadrilaterals(image_with_failed_angles, rectangles, (255, 255, 255))  # rectángulos válidos en blanco
        # Dibuja trapecios con ángulos fallidos en otro color, por ejemplo verde
        self.draw_quadrilaterals(image_with_failed_angles, failed_angle_traps, (0, 128, 0))  # verde para fallidos
        ...
        cv2.imwrite(os.path.join(output_folder, f"{base_name}_debug_graph_black.png"), graph_black)
        cv2.imwrite(os.path.join(output_folder, f"{base_name}_rects_plus_failed_angles.png"), image_with_failed_angles)

        return output, rectangles, failed_angle_traps
...
def main(input_folder, output_folder):
    ...
    detections_combined = {}  # Solo rectángulos válidos
    ...
    for filename in os.listdir(input_folder):
        if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp')):
            ...
            output_img, rects, failed_angle_traps = detector.process_image(image, output_folder, base_name)

            output_path = os.path.join(output_folder, filename)
            cv2.imwrite(output_path, output_img)

            valid_rects = [[[int(x), int(y)] for [x, y] in rect] for rect in rects]

            # Solo guardamos los rectángulos válidos, no los fallidos
            detections_combined[filename] = valid_rects

            print(f"Guardado: {output_path}")
            print(f"Total válidos: {len(valid_rects)}\n")

    combined_path = os.path.join(output_folder, "detections_combined_rectangles.json")
    with open(combined_path, "w") as f:
        json.dump(detections_combined, f, indent=4)



if __name__ == "__main__":
    input_folder = r"C:\Users\User\Desktop\Nueva carpeta MASTER\RED_TRAIN"
    output_folder = r"C:\Users\User\Desktop\Nueva carpeta MASTER\Wireframe-20250303T172646Z-001\Wireframe\resul115"
